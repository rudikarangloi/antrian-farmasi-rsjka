    		...
    		if($rowClient['count']){
				...
				*/			
				...
				if($rows['count']){					
					...
					if($repeatId != 0){
						$sqlb = "UPDATE data_antrian_apotik SET status_obat = 1 WHERE STATUS=1 AND nomor=$repeatId;";
					...
					$results = $mysqli->query('UPDATE data_antrian_apotik SET status_obat = 1 WHERE id='.$idt.'');
				...
				if($repeatId != 0){
					...
					$sqlb = "UPDATE data_antrian_apotik SET status_obat = 1 WHERE id=$idt ". $sql_loket;
